package com.kfd.cloudgallery.manager.auth;
...
import static com.kfd.cloudgallery.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        ...
        if (spaceId == null) {
            ...
            // 检查是否是同步图像（ID在1000000-2000000范围内，且减去1000000后在image_data_sync表中存在）
            if (pictureId >= 1000000L && pictureId < 2000000L) {
                // 这可能是同步图像，先尝试查询image_data_sync表
                Long syncImageId = pictureId - 1000000L;
                ImageDataSync syncImage = imageDataSyncService.getById(syncImageId);
                if (syncImage != null) {
                    // 确认是同步图像，默认允许所有用户查看（公共图库）
                    return ADMIN_PERMISSIONS;
                }
            }
            
            // 不是同步图像或同步图像不存在，按普通图像处理
